...
export function TemplatePoster({ template, onClick }: { template: Template; onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group relative w-full overflow-hidden rounded-[24px] border border-[hsl(var(--color-border)/0.65)] bg-[hsl(var(--color-surface)/0.45)] text-left shadow-[var(--shadow-soft)] transition hover:-translate-y-0.5 hover:border-[hsl(var(--color-accent)/0.35)] hover:shadow-[var(--shadow-hard)]"
    >
      <div className="relative aspect-[5/4] overflow-hidden bg-[hsl(var(--color-elevated))]">
        ...
        <div className="absolute inset-x-3 bottom-3">
          <p className="text-base font-semibold text-white drop-shadow-sm">{template.name}</p>
          <p className="mt-1 line-clamp-2 text-xs text-white/76">{template.short_description || template.description}</p>
        </div>
      </div>
    </button>
  );
