...
export function TemplatePoster({ template, onClick }: { template: Template; onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group relative w-full overflow-hidden rounded-[18px] border border-[hsl(var(--color-border)/0.65)] bg-[hsl(var(--color-surface)/0.45)] text-left shadow-[var(--shadow-soft)] transition hover:-translate-y-0.5 hover:border-[hsl(var(--color-accent)/0.35)] hover:shadow-[var(--shadow-hard)]"
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-[hsl(var(--color-elevated))]">
        ...
        <div className="absolute inset-x-2.5 bottom-2.5">
          <p className="text-sm font-semibold text-white drop-shadow-sm">{template.name}</p>
          <p className="mt-1 line-clamp-2 text-[11px] text-white/76">{template.short_description || template.description}</p>
        </div>
      </div>
    </button>
  );
