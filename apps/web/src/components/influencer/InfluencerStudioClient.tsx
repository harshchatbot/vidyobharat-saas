...
import { Camera, ImageIcon, Layers3, Lock, RefreshCw, Sparkles, UserRound, Wand2 } from 'lucide-react';
...
export function InfluencerStudioClient({ userId }: { userId: string }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-[var(--radius-lg)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))] px-5 py-6 shadow-soft sm:px-6">
      ...
      <div className="flex gap-2 overflow-x-auto rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))] p-2">
        {tabItems.map((item) => (
          <button
            key={item.key}
            type="button"
            ...
            <div className="grid gap-4">
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div>
                  ...
                  <Dropdown value={selectedImageModel} onChange={(e) => setSelectedImageModel(e.target.value)}>
                    {imageModels.map((model) => (
                      <option key={model.key} value={model.key}>{model.label}</option>
                    ))}
                  </Dropdown>
                  <p className="mt-2 text-xs text-muted">Choose the model based on how polished or stylized you want the final portrait to feel.</p>
                </div>
                <div>
