...
class User(Base):
    ...
    influencer_personas: Mapped[list['InfluencerPersona']] = relationship(back_populates='user')
...
class Video(Base):
    ...
    audio_sample_rate_hz: Mapped[int] = mapped_column(Integer, default=48000)
...
class CreditTopUpOrder(Base):
    ...
    plan_name: Mapped[str] = mapped_column(String(32), default='starter')
    pricing_region: Mapped[str] = mapped_column(String(24), default='south_asia')
    credits: Mapped[int] = mapped_column(Integer)
    amount_paise: Mapped[int] = mapped_column(Integer)  # Legacy minor-unit column retained for backward compatibility.
...
class InfluencerPersona(Base):
    __tablename__ = 'influencer_personas'
    ...
    catchphrase: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ...
    reference_image_url: Mapped[str | None] = mapped_column(String(255), nullable=True)
    reference_image_path: Mapped[str | None] = mapped_column(String(255), nullable=True)
    reference_embedding_vector: Mapped[str | None] = mapped_column(Text, nullable=True)
    style_embedding_vector: Mapped[str | None] = mapped_column(Text, nullable=True)
    system_prompt_template: Mapped[str | None] = mapped_column(Text, nullable=True)
    character_locked: Mapped[bool] = mapped_column(default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())

    user: Mapped['User'] = relationship(back_populates='influencer_personas')
    scene_presets: Mapped[list['InfluencerScenePreset']] = relationship(back_populates='persona')


class InfluencerScenePreset(Base):
    __tablename__ = 'influencer_scene_presets'

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id: Mapped[str] = mapped_column(ForeignKey('users.id', ondelete='CASCADE'), index=True)
    persona_id: Mapped[str | None] = mapped_column(ForeignKey('influencer_personas.id', ondelete='CASCADE'), nullable=True, index=True)
    ...
    persona: Mapped['InfluencerPersona | None'] = relationship(back_populates='scene_presets')
