...
class StoryboardPipelineService:
    ...
    def _enrich_scenes_with_category_metadata(self, scenes: list[Any], business_brief: str) -> list[dict[str, Any]]:
        """Add product_category and scene position metadata to each scene."""
        product_category = detect_product_category(business_brief)
        total_scenes = len(scenes)

        enriched = []
        for i, scene in enumerate(scenes):
            scene_dict = dict(scene) if hasattr(scene, '__dict__') else scene
            scene_dict['product_category'] = product_category
            scene_dict['scene_index'] = i
