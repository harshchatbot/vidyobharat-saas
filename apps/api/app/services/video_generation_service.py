...
class VideoGenerationService:
    ...
    def generate_talking_avatar_clip(self, request: ClipGenerationRequest):
        ...
        normalized_voice_provider = str(request.voice_provider or '').strip().lower() or 'sarvam'
        fallback_metadata = {
        ...
        if normalized_persona_provider == 'heygen':
            raise RuntimeError(
                f'Selected avatar "{request.persona_id}" is still configured for the retired HeyGen path. '
                'Please re-save or recreate it as a standard reference-image avatar.'
            )
