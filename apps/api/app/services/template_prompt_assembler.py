...
class TemplatePromptAssembler:
    ...
    def _motion_block(self, template_type: str | None) -> str:
        if template_type != 'video':
            return ''
        return (
            'Keep motion coherent and transitions intentional. Start with a natural, non-abrupt opening beat or gentle fade-in, '
            'maintain smooth scene-to-scene flow, and finish with a clean ease-out or held final frame instead of an abrupt stop.'
        )
    ...
    def _generic_script(self, template: UnifiedTemplateResponse, prompt: str, values: dict[str, str]) -> str:
        ...
        return (
            ...
            '[Motion] Open gently with smooth camera movement or a soft fade-in, not an abrupt first frame.\n\n'
            f'[Middle] Deliver the core insight with platform-aware scenes, cutaways, on-screen text, and smooth visual transitions.\n'
            f'[Close] End with a strong CTA: {cta} Use a natural ease-out or held final frame with no abrupt cut.'
        )
    ...
    def _assemble_character_explainer_reel(self, template: UnifiedTemplateResponse, values: dict[str, str]) -> PromptAssemblyResult:
        ...
        script_preview = self._scene_script(
            ('Hook', f'Narrator ({language}, {tone}): "{subtype_hook}" Open with a gentle visual settle-in, not an abrupt cut.'),
            ...
            ('Close', f'End with a clean CTA: "{cta}." Use a natural ease-out or held final composition so the ending feels smooth, not abrupt.'),
        )
    ...
    def _assemble_viral_dance_clip(self, template: UnifiedTemplateResponse, values: dict[str, str]) -> PromptAssemblyResult:
        ...
        script_preview = self._scene_script(
            ('Hook', f'Open on {subject} already in motion with a strong first-frame pose that matches a {music_mood} sound bed, but let the entrance feel naturally eased in rather than abrupt.'),
            ...
            ('Loop Close', f'Return to a pose or motion that flows back into the opening frame for a {loop}. The last beat should feel seamless, not abruptly chopped.'),
        )
        return PromptAssemblyResult(prompt, video_prompt=prompt, script_preview=script_preview, recommended_model_mode=template.default_model_mode)
    ...
    def _assemble_client_ad_reel(self, template: UnifiedTemplateResponse, values: dict[str, str]) -> PromptAssemblyResult:
        ...
        script_preview = self._scene_script(
            ('Hook', f'Open with the headline "{headline}" and frame the problem for {audience} in under two seconds, using a smooth visual lead-in rather than an abrupt jump.'),
            ...
            ('Close', f'Land the CTA strongly: "{cta}." End on branded composition, a clear action, and a natural ease-out with no abrupt stop.'),
        )
        return PromptAssemblyResult(prompt, video_prompt=prompt, script_preview=script_preview, recommended_model_mode=template.default_model_mode)
    ...
    def _assemble_story_slides_reel(self, template: UnifiedTemplateResponse, values: dict[str, str]) -> PromptAssemblyResult:
        ...
        subtype_body = {
            'top_5': (
                'Hook',
                f'Open with a rank-based hook around {topic}, then move through the list with one strong visual claim per slide. Let the first slide settle in smoothly rather than popping in abruptly.'
            ),
            'timeline': (
                'Hook',
                f'Open with the first key moment in the timeline of {topic}, then progress chronologically with clean year/event transitions and a smooth first reveal.'
            ),
            'before_after': (
                'Hook',
                f'Open with a dramatic before/after contrast around {topic}, then reveal the transformation through two to three visual beats with smooth pacing.'
            ),
            'mini_story': (
                'Hook',
                f'Open with one emotionally charged first frame about {topic}, then resolve the arc across a few slides with a natural lead-in.'
            ),
        ...
        script_preview = self._scene_script(
            ...
            ('Close', f'End with a final CTA slide: "{cta}." Let the closing slide hold briefly or ease out naturally so the ending does not feel abrupt.'),
        )
        return PromptAssemblyResult(prompt, video_prompt=prompt, script_preview=script_preview, recommended_model_mode=template.default_model_mode)
